import java.util.Iterator;
import java.util.NoSuchElementException;

import components.map.Map;
import components.map.Map2;
import components.map.MapSecondary;

/**
 * {@code Map} represented as a hash table using {@code Map}s for the buckets,
 * with implementations of primary methods.
 *
 * @param <K>
 *            type of {@code Map} domain (key) entries
 * @param <V>
 *            type of {@code Map} range (associated value) entries
 * @convention <pre>
 * |$this.hashTable| > 0  and
 * for all i: integer, pf: PARTIAL_FUNCTION, x: K
 *     where (0 <= i  and  i < |$this.hashTable|  and
 *            <pf> = $this.hashTable[i, i+1)  and
 *            x is in DOMAIN(pf))
 *   ([computed result of x.hashCode()] mod |$this.hashTable| = i))  and
 * for all i: integer
 *     where (0 <= i  and  i < |$this.hashTable|)
 *   ([entry at position i in $this.hashTable is not null])  and
 * $this.size = sum i: integer, pf: PARTIAL_FUNCTION
 *     where (0 <= i  and  i < |$this.hashTable|  and
 *            <pf> = $this.hashTable[i, i+1))
 *   (|pf|)
 * </pre>
 * @correspondence <pre>
 * this = union i: integer, pf: PARTIAL_FUNCTION
 *            where (0 <= i  and  i < |$this.hashTable|  and
 *                   <pf> = $this.hashTable[i, i+1))
 *          (pf)
 * </pre>
 *
 * @author Shyam Sai Bethina and Yihone Chu
 *
 */
public class Map4<K, V> extends MapSecondary<K, V> {

    /*
     * Private members --------------------------------------------------------
     */

    /**
     * Default size of hash table.
     */
    private static final int DEFAULT_HASH_TABLE_SIZE = 101;

    /**
     * Buckets for hashing.
     */
    private Map<K, V>[] hashTable;

    /**
     * Total size of abstract {@code this}.
     */
    private int size;

    /**
     * Computes {@code a} mod {@code b} as % should have been defined to work.
     *
     * @param a
     *            the number being reduced
     * @param b
     *            the modulus
     * @return the result of a mod b, which satisfies 0 <= {@code mod} < b
     * @requires b > 0
     * @ensures <pre>
     * 0 <= mod  and  mod < b  and
     * there exists k: integer (a = k * b + mod)
     * </pre>
     */
    public static int mod(int a, int b) {
        int answer = 0;

        /*
         * If the modded number is greater than 0, then the answer is the normal
         * result of the remainder of a/b.
         */
        if (a > 0) {
            answer = a % b;
        } else {

            /*
             * If the modded number is negative, we find the mod the usual way,
             * but add the result by the modding number according to clock
             * arithmetic.
             */

            int toBeAdded = a % b;
            if (toBeAdded != 0) {
                answer = toBeAdded + b;
            }
        }
        return answer;
    }

    /**
     * Creator of initial representation.
     *
     * @param hashTableSize
     *            the size of the hash table
     * @requires hashTableSize > 0
     * @ensures <pre>
     * |$this.hashTable| = hashTableSize  and
     * for all i: integer
     *     where (0 <= i  and  i < |$this.hashTable|)
     *   ($this.hashTable[i, i+1) = <{}>)  and
     * $this.size = 0
     * </pre>
     */
    @SuppressWarnings("unchecked")
    private void createNewRep(int hashTableSize) {
        /*
         * With "new Map<K, V>[...]" in place of "new Map[...]" it does not
         * compile; as shown, it results in a warning about an unchecked
         * conversion, though it cannot fail.
         */

        /*
         * The size is 0 by default
         */
        this.size = 0;

        /*
         * this.hasTable becomes an array of maps
         */
        this.hashTable = new Map[hashTableSize];

        /*
         * Goes through each empty array index and initializes a new map in it
         */
        for (int i = 0; i < this.hashTable.length; i++) {
            this.hashTable[i] = new Map2<K, V>();
        }

    }

    /*
     * Constructors -----------------------------------------------------------
     */

    /**
     * No-argument constructor.
     */
    public Map4() {

        /*
         * Constructs an array of maps using the default hash table size
         */
        this.createNewRep(this.DEFAULT_HASH_TABLE_SIZE);

    }

    /**
     * Constructor resulting in a hash table of size {@code hashTableSize}.
     *
     * @param hashTableSize
     *            size of hash table
     * @requires hashTableSize > 0
     * @ensures this = {}
     */
    public Map4(int hashTableSize) {

        /*
         * Constructs an array of maps of size hasTableSize
         */
        this.createNewRep(hashTableSize);
    }

    /*
     * Standard methods -------------------------------------------------------
     */

    @SuppressWarnings("unchecked")
    @Override
    public final Map<K, V> newInstance() {
        try {
            return this.getClass().getConstructor().newInstance();
        } catch (ReflectiveOperationException e) {
            throw new AssertionError(
                    "Cannot construct object of type " + this.getClass());
        }
    }

    @Override
    public final void clear() {
        this.createNewRep(DEFAULT_HASH_TABLE_SIZE);
    }

    @Override
    public final void transferFrom(Map<K, V> source) {
        assert source != null : "Violation of: source is not null";
        assert source != this : "Violation of: source is not this";
        assert source instanceof Map4<?, ?> : ""
                + "Violation of: source is of dynamic type Map4<?,?>";
        /*
         * This cast cannot fail since the assert above would have stopped
         * execution in that case: source must be of dynamic type Map4<?,?>, and
         * the ?,? must be K,V or the call would not have compiled.
         */
        Map4<K, V> localSource = (Map4<K, V>) source;
        this.hashTable = localSource.hashTable;
        this.size = localSource.size;
        localSource.createNewRep(DEFAULT_HASH_TABLE_SIZE);
    }

    /*
     * Kernel methods ---------------------------------------------------------
     */

    @Override
    public final void add(K key, V value) {
        assert key != null : "Violation of: key is not null";
        assert value != null : "Violation of: value is not null";
        assert !this.hasKey(key) : "Violation of: key is not in DOMAIN(this)";

        /*
         * Mods the hash code of key by the hashTable's length and sets it to
         * result. Then the key and value are added as a pair to the index in
         * the array which is determinded by result.
         */
        int result = mod(key.hashCode(), this.hashTable.length);

        this.hashTable[result].add(key, value);

    }

    @Override
    public final Pair<K, V> remove(K key) {
        assert key != null : "Violation of: key is not null";
        assert this.hasKey(key) : "Violation of: key is in DOMAIN(this)";

        /*
         * Mods the hash code of key by the hashTable's length and sets it to
         * result. Then the key and value at the index of result in the array
         * are removed as a pair and returned.
         */
        int result = mod(key.hashCode(), this.hashTable.length);

        Map.Pair<K, V> pair = this.hashTable[result].remove(key);
        return pair;
    }

    @Override
    public final Pair<K, V> removeAny() {
        assert this.size() > 0 : "Violation of: this /= empty_set";

        /*
         * Initializes stop to use to stop the while loop.
         */
        boolean stop = false;

        int i = 0;

        /*
         * The while loops keeps going until a "bucket" in the array is not
         * empty. When the loop finds a non-empty index, int i becomes the
         * index.
         */
        while (!stop || i == this.hashTable.length) {
            if (this.hashTable[i].size() != 0) {
                stop = true;

            }
            i++;
        }

        /*
         * Removes and returns any pair from the map pairs in the ith index of
         * the array.
         */
        Map.Pair<K, V> pair = this.hashTable[i - 1].removeAny();

        return pair;
    }

    @Override
    public final V value(K key) {
        assert key != null : "Violation of: key is not null";
        assert this.hasKey(key) : "Violation of: key is in DOMAIN(this)";

        /*
         * Mods the hash code of key by the hashTable's length and sets it to
         * result. Then the value of the key at the index of result in the array
         * is returned.
         */
        int result = mod(key.hashCode(), this.hashTable.length);
        V value = this.hashTable[result].value(key);

        return value;
    }

    @Override
    public final boolean hasKey(K key) {
        assert key != null : "Violation of: key is not null";

        /*
         * Mods the hash code of key by the hashTable's length and sets it to
         * result.
         */
        int result = mod(key.hashCode(), this.hashTable.length);

        /*
         * Initializes hasKey to return the result of looping through the pairs
         */
        boolean hasKey = false;

        /*
         * The loop goes through all the pairs at the index of the array and
         * checks to see if any of the pair's key matches the given key. If so,
         * hsaKey is set to true. If not, hasKey stays false.
         */
        for (Pair<K, V> temp : this.hashTable[result]) {
            if (temp.key().equals(key)) {
                hasKey = true;
            }
        }

        return hasKey;
    }

    @Override
    public final int size() {
        int answer = 0;

        /*
         * Increments answer by one for every pair in this.hashTable.
         */
        for (Map<K, V> temp : this.hashTable) {
            answer += temp.size();
        }

        return answer;
    }

    @Override
    public final Iterator<Pair<K, V>> iterator() {
        return new Map4Iterator();
    }

    /**
     * Implementation of {@code Iterator} interface for {@code Map4}.
     */
    private final class Map4Iterator implements Iterator<Pair<K, V>> {

        /**
         * Number of elements seen already (i.e., |~this.seen|).
         */
        private int numberSeen;

        /**
         * Bucket from which current bucket iterator comes.
         */
        private int currentBucket;

        /**
         * Bucket iterator from which next element will come.
         */
        private Iterator<Pair<K, V>> bucketIterator;

        /**
         * No-argument constructor.
         */
        Map4Iterator() {
            this.numberSeen = 0;
            this.currentBucket = 0;
            this.bucketIterator = Map4.this.hashTable[0].iterator();
        }

        @Override
        public boolean hasNext() {
            return this.numberSeen < Map4.this.size;
        }

        @Override
        public Pair<K, V> next() {
            assert this.hasNext() : "Violation of: ~this.unseen /= <>";
            if (!this.hasNext()) {
                /*
                 * Exception is supposed to be thrown in this case, but with
                 * assertion-checking enabled it cannot happen because of assert
                 * above.
                 */
                throw new NoSuchElementException();
            }
            this.numberSeen++;
            while (!this.bucketIterator.hasNext()) {
                this.currentBucket++;
                this.bucketIterator = Map4.this.hashTable[this.currentBucket]
                        .iterator();
            }
            return this.bucketIterator.next();
        }

        @Override
        public void remove() {
            throw new UnsupportedOperationException(
                    "remove operation not supported");
        }

    }

}
